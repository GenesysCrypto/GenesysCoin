...
void Shutdown(void* parg)
{
    ...
    RenameThread("GenesysCoin-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("GenesysCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  genesysd [options]                     " + "\n" +
                  "  genesysd [options] <command> [params]  " + _("Send command to -server or genesysd") + "\n" +
                  "  genesysd [options] help                " + _("List commands") + "\n" +
                  "  genesysd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "GenesysCoin:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("GenesysCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("GenesysCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: genesys.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: genesysd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 37020 or testnet: 37021)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 36020 or testnet: 36021)") + "\n" +
		...
		strUsage += " -anonymizeGenesysCoinamount=<n> " + _("Keep N GenesysCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  GenesysCoin is probably already running."), strDataDir.c_str()));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("GenesysCoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "GenesysCoin server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("GenesysCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (mapArgs.count("-reservebalance")) // GenesysCoin: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // GenesysCoin: checkpoint master priv key
    {
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of GenesysCoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart GenesysCoin to complete") << "\n";
	...
	nAnonymizeGenesysCoinAmount = GetArg("-anonymizeGenesysCoinamount", 0);
	if(nAnonymizeGenesysCoinAmount > 999999) nAnonymizeGenesysCoinAmount = 999999;
	if(nAnonymizeGenesysCoinAmount < 2) nAnonymizeGenesysCoinAmount = 2;
	...
	if(nInstantXDepth < 0) nAnonymizeGenesysCoinAmount = 0;
	} else {
	nInstantXDepth = 0;
	}
	...
	printf("Anonymize GenesysCoin Amount %d\n", nAnonymizeGenesysCoinAmount);

	/* Denominations
	A note about convertability. Within Darksend pools, each denomination
	is convertable to another.
	For example:
	1GenesysCoin+1000 == (.1GenesysCoin+100)*10
	10GenesysCoin+10000 == (1GenesysCoin+1000)*10
	*/
