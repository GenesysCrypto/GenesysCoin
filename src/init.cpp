...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: genesys.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: genesysd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 37020 or testnet: 37021)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 36020 or testnet: 36021)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizegenesysamount=<n> " + _("Keep N GenesysCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. GenesysCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GenesysCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("GenesysCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeGenesysCoinAmount = GetArg("-anonymizegenesysamount", 0);
    ...
    LogPrintf("Anonymize GenesysCoin Amount %d\n", nAnonymizeGenesysCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1GSY+1000 == (.1GSY+100)*10
       10GSY+10000 == (1GSY+1000)*10
    */
    ...
    */
	...
	{
	    uiInterface.NotifySkyNodeChanged(adrenaline.second);
	}
