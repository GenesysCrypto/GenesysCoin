    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s GSY %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s GSY %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
           ...
           /* for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
					...
					if(found && coin_type == ONLY_NONDENOMINATED_NOTMN) found = (pcoin->vout[i].nValue != 2000*COIN); // do not use MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
...
bool CWallet::SelectCoins(int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1GSY
    if(coin_type == ONLY_DENOMINATED){
        ...
        BOOST_FOREACH(int64_t v, darkSendDenominations)
        {
            ...
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1GSY over
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, vector<COutput>& setCoinsRet2, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fMasterNode && out.tx->vout[out.i].nValue == 2000*COIN) continue; //masternode input 2000GSY
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            bool fAccepted = false;*/ //GetMN

            // Function returns as follows:
            //
            // bit 0 - 100GSY+1 ( bit on if present )
            // bit 1 - 10GSY+1
            // bit 2 - 1GSY+1
            // bit 3 - .1GSY+1
...
bool CWallet::SelectCoinsDark(int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        //there's no reason to allow inputs less than 1 COIN into DS (other than denominations smaller than that amount)
        if(out.tx->vout[out.i].nValue < 1*COIN && out.tx->vout[out.i].nValue != (.1*COIN)+100) continue;
        /*if(fMasterNode && out.tx->vout[out.i].nValue == GetMNCollateral(pindexBest->nHeight)*COIN) continue; //masternode input*/ //GetMN
		if(fMasterNode && out.tx->vout[out.i].nValue == 2000*COIN) continue; //masternode input 2000GSY
...
{
    /*return  nInputAmount != 0 && nInputAmount % DARKSEND_COLLATERAL == 0 && nInputAmount < DARKSEND_COLLATERAL * 5;*/ //MNOrig
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough Darksend non-denominated funds for this transaction that are not equal 1000 GSY.");
                    } else {
		    ...
		    nChangePos = std::distance(wtxNew.vout.begin(), position);
                ...
                //int64_t nMinFee = GetMinFee(wtxNew, 1, fAllowFree, GMF_SEND, nBytes);
...
uint64_t CWallet::GetStakeWeight() const
{
	...
	int64_t nCurrentTime = GetTime(); //new code added
    CTxDB txdb("r");
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s GSY %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s GSY %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
bool CWallet::AddSkyNodeConfig(CSkyNodeConfig nodeConfig)
{
    bool rv = CWalletDB(strWalletFile).WriteSkyNodeConfig(nodeConfig.sAlias, nodeConfig);
    if(rv)
	uiInterface.NotifySkyNodeChanged(nodeConfig);
