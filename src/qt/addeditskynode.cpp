...
AddEditSkyNode::AddEditSkyNode(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddEditSkyNode)
{
    ui->setupUi(this);

}

AddEditSkyNode::~AddEditSkyNode()
{
    delete ui;
}


void AddEditSkyNode::on_okButton_clicked()
{
	...
	bool bForceNew = false;
        ...
        pwalletMain->mapMySkyNodes.insert(make_pair(c.sAddress, c));
	walletdb.WriteSkyNodeConfig(c.sAddress, c);
        uiInterface.NotifySkyNodeChanged(c);

        accept();
    }
}

void AddEditSkyNode::on_cancelButton_clicked()
{
    reject();
}
