...
{
    //resize(970, 560);
	setFixedSize(975, 565);
    setWindowTitle(tr("Genesys Standard") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    //setObjectName("genesyscoin");
    //setStyleSheet("#genesyscoin { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
	
	/* -----------------------------Style Sheet-----------Start----------------------------- */
	
	qApp->setStyleSheet("QMainWindow 		{ background-color: #cccccc;border:none; }"
	
	//"QProgressBar   { color: rgb(204,204,204); border-color: rgb(204,204,204); border-width: 2px; border-style: solid; }"
    //"QProgressBar::chunk { background-color: rgb(142,7,11); }"
	
	"#labelMiningIcon { padding-left:5px;font-family:none;width:100%;font-size:7px;text-align:center;color: rgb(204,204,204); } QMenu { background: rgb(39,39,39); color:white; padding-bottom:10px; } QMenu::item { color: rgb(204,204,204); background-color: transparent; } QMenu::item:selected { background-color:qlineargradient(x1: 0, y1: 0, x2: 0.5, y2: 0.5,stop: 0 rgb(36,36,36), stop: 1 rgb(142,7,11)); } QMenuBar { background: rgb(39,39,39); color:white; border-bottom: 0px solid rgb(230,0,0); } QMenuBar::item { font-size:7px;padding-bottom:12px;padding-top:12px;padding-left:15px;padding-right:15px;color: rgb(204,204,204); background-color: transparent; } QMenuBar::item:selected { background-color:qlineargradient(x1: 0, y1: 0, x2: 0.5, y2: 0.5,stop: 0 rgb(39,39,39), stop: 1 rgb(142,7,11)); }");
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a GenesysCoin address"));
    ...
    masternodeManagerAction = new QAction(QIcon(":/icons/skynode"), tr("&SkyNodes"), this);
    masternodeManagerAction->setToolTip(tr("Show your SkyNodes status and configuration."));
    ...
    aboutAction = new QAction(tr("&About GenesysCoin"), this);
    aboutAction->setToolTip(tr("Show information about GenesysCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for GenesysCoin"));
...
void BitcoinGUI::createToolBars()
{
    ...
    toolbar->setStyleSheet("QToolButton { color: #cccccc; font-weight:bold;} QToolButton:hover { background-color: #8e070b } QToolButton:checked { background-color: #d70b11 } QToolButton:pressed { background-color: #d70b11} #tabs { color: #cccccc; background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 0.6, stop: 0 #8e070b, stop: 1 #8e070b);  }");
	
	QLabel* header2 = new QLabel();
	header2->setMinimumSize(160, 40);
	header2->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    header2->setPixmap(QPixmap(":/images/header2"));
	header2->setScaledContents(true);
	toolbar->addWidget(header2);
    ...
    netLabel->setStyleSheet("#netLabel { color: #cccccc; }");
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("GenesysCoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("GenesysCoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to GenesysCoin network", "", count));
}
...
void BitcoinGUI::setNumBlocks(int count)
{
    ...
    else
    {
        ...
        progressBarLabel->setStyleSheet("QLabel { color: #8e070b; }");
...
{
    QString strTitle = tr("GenesysCoin") + " - ";
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid GenesysCoin address or malformed URI parameters."));
    }
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    if (nLastCoinStakeSearchInterval && nWeight)
    {
		...
		if(pindexBest->nHeight <= nTARGET_SPACING_2_SWITCH_BLOCK){
            ...
            nEstimateTime = nTARGET_SPACING_2 * nNetworkWeight / nWeight;
        }
