...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    {
        nSubsidy = 1000000 * COIN;   // For GenesysCoin CrowdFund
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
		...
		else if(pindexBest->nHeight < 152001)
    {
		...
		else if(pindexBest->nHeight < 322000)
    {
        nSubsidy = 0.25 * COIN;  // ~25000 (~80days Staking)
    }
		else if(pindexBest->nHeight < 422001)
    {
        nSubsidy = 0.175 * COIN;  // ~17500 (~80days Staking)
    }
    
	// Fork to permanent block rewards halving to 0.149
		else if(pindexBest->nHeight > 422000)
    {
        nSubsidy = 0.149 * COIN;  // 0.149 Coins/Stake: 70% (0.1043 Coins) for Skynodes and 30% (0.0447 Coins) for Wallet Clients
    }
...
{
    RenameThread("genesyscoin-loadblk");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        //if (fSecMsgEnabled)
            //SecureMsgScanBlock(block); //no need
    }
