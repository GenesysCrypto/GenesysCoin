    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat GenesysCoin and make it unusable. To
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    if(!sessionFoundMasternode){
        ...
        double fGenesysCoinSubmitted = (sessionTotalValue / COIN);
        LogPrintf("Submitting Darksend for %f GSY COIN - sessionTotalValue %d\n", fGenesysCoinSubmitted, sessionTotalValue);
...
void ThreadCheckDarkSendPool()
{
    ...
    while (true)
    {
        ...
        if(c % 5 == 0 && RequestedMasterNodeList < 10){		// 10 nodes
