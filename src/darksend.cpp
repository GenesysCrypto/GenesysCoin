    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat GSY and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002GSY per transaction on average.
            */
            if(r <= 20)
            {
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fGSYSubmitted = (sessionTotalValue / CENT);
        printf("Submitting Darksend for %f GSY CENT - sessionTotalValue %d\n", fGSYSubmitted, sessionTotalValue);
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey){
    ...
    if(GetTransaction(vin.prevout.hash, txVin, hash)){
        BOOST_FOREACH(CTxOut out, txVin.vout){
            if(out.nValue == 2000*COIN){                                               // 2000MN
...
void ThreadCheckDarkSendPool(void* parg)
{
    ...
    RenameThread("GenesysCoin-darksend");
    ...
    while (true)
    {
        ...
        //try to sync the masternode list and payment list every 5 seconds from at least 8 nodes
        if(c % (5*5) == 0 && RequestedMasterNodeList < 12){      //// Old ....< 8
        ...
        if(c % (60*5)*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeGenesysCoinAmount = (rand() % nLeftToAnon)+3;
                } else {
